...
def determine_player(history):
    n = 0 

    if (flop_happened(history)):
        hist_parts = reversed(history.split(" "))
        for part in hist_parts:
            if part in init_deck.card_dictionary:
                break 
            n += 1
...
def main():
    ...
    for _ in range(n_iterations):
        ...
        if k != 0:  # FOR EVERY OTHER ITERATION WHERE WE ALREADY HAVE A GENERATED IMAP 
            return_tuple = cfr(i_map)
            i_map = return_tuple[1]
            expected_game_value+=return_tuple[0]

        iter_end = time.time()
        print ("ITERATION COMPLETE " + str(k+1) + "/" + str(n_iterations) + " IN " + str(iter_end-iter_start) + " SECONDS ")
        k = k+1
...
def cfr(i_map, history="", p1_card1=-1,p1_card2 =-1, p2_card1=-1,  p2_card2=-1, pr_1=1, pr_2=1, pr_c=1):
    ...
    if is_dealing_chance(history):
        if k == 0: 
            ...
            return dealing_util_i_map_generated(i_map)
...
def dealing_util_i_map_generated(i_map):
    ...
    for combo_1 in card_combos:
        card_1 = init_deck.card_to_string(list(combo_1)[0])
        card_2 = init_deck.card_to_string(list(combo_1)[1])
        string_filter = str(card_1 + " " +card_2)
        filtered_local_map = {key: value for key, value in i_map.items() if key.startswith(string_filter)}
        local_map_list.append(filtered_local_map)


    with ProcessPoolExecutor(max_workers=30) as executor:
        map_list = list(executor.map(calculate_expected_value, local_map_list, card_combos))
...
def display_results(ev, i_map):
    file = open("cfr_eval_1_iterations_shared_map.txt", "w")
